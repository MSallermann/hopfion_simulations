...
width_ratios = np.ones(8)
width_ratios[-1] = 2/3
gs0.set_width_ratios(width_ratios)
...
# Surfaces top
for col,a in enumerate(pplot.row(0, slice(0,-1), gs=gs0)):
    gamma = float(1/7*col)
    l0    = float(5.00)
...
# # Surfaces right
for row,a in enumerate(pplot.col(-1, slice(0,None,None), gs=gs0)):
    gamma = float( 1 )
    l0    = float( 5 - (row) * 0.5 )
    file = os.path.join(SCRIPT_DIR, "renderings", NAME +  f"_{gamma:.3f}_{l0:.3f}_{SUFFIX}.png")
    print(file)
    ...
    width = image.shape[1]
    image = pplot.crop(image, int(width*2/3) )

    pplot.image_to_ax(a, image)
    # a.axis("off")
...
fig.savefig("plot1_v5.png", dpi=300)